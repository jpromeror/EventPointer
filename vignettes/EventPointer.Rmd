...
abstract: >
  *EventPointer* is an R package to identify alternative splicing events in complex experimental designs such as time course and paired samples studies. The algorithm can be used to analyze data from either junction arrays (Affymetrix Arrays) or sequencing data (RNA-Seq). 
...
Files 1 and 2 contain probe information for the array. These files can be downloaded from the following URL: [EventPointer Dropbox](https://www.dropbox.com/sh/wpwz1jx0l112icw/AAAOU0M2QS0TjqHAhrrdEhp6a?dl=0)
...
For the examples shown in this vignette, the R package includes a series of example files to run the corresponding functions. However, to build the complete CDF , the complete files must be used. 

<span style="color:red">**Note:**</span> It is advisable to work with reference GTF files, as the probes are annotated to it. However, if other database is used, the algorithm will only include probes that are mapped to such database.
...
- **input** : Reference transcriptome. Available options are : "Ensembl","UCSC" and "GTF".
...
# Generate Visualization files  
 ...
 EventPointer_IGV(Events[1,,drop=FALSE],"GTF",DONSON_GTF,PSRProbes,JunctionProbes,Directory,EventsFound,array)
