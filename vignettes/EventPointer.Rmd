...
*EventPointer* is prepared to work with the Affymetrix Human Transcriptome Array 2.0 (HTA 2.0). As newer arrays are developed, EP will be able to work with them, as long the required files are availble for download. 

To build the CDF file (to work under the aroma.affymetrix framework), EP requires a GTF file with the reference transcriptome information. In case not provided, the algorithm automatically downloads the requried information from different databases such as ENSEMBL or UCSC. As the probes for the HTA 2.0 array are mapped to the HG19 genome, the latests versions of the ensembl and ucsc genome, mapped to the hg19 version, will be downloaded.

The default required files can be downloaded from the [EventPointer Github repository](https://github.com/jpromeror/EventPointer) :

1. Exon probes genomic position
2. Junction probes genomic position
3. HTA 2.0 reference transcriptome (GTF)
...
The following code chunks show examples on how to run the function using both Ensembl and Affymetrix GTF using
