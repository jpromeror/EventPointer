...
![**Figure 7.** Analysis of RNA-Seq data with kallisto output ](Esquema_bis.png)
...
* **EventsFound_<Transcriptome>.txt** A .txt file with the information of the events.
...
The following code shows an example of how to read the kallisto output and prepare it to run the main function.
...
kable(Fit,digits=5,row.names=TRUE,align="c",caption = "Table 3: PSI_Statistic results")
```
...
The function requires the following main parameters:
...
- **EventNum**: Specific information of the required event. Basically, each event is composed of two exclusive paths and a common reference for both paths.
...
From the output data.frame we get the sequence of the primers and probes if it is needed. We can visualize the result in IGV using the *Find Motif* tool.
