...
The following code chunks show examples on how to run the function using part of the Affymetrix GTF and the example data included in the package. This data corresponds to the Affymetrix HTA 2.0 GTF represnting only the DONSON gene and the probes that are mapped to it.
...
For microarray dataa, a pre-processing pipeline must be applied to the .cel files of the experiment.Usually this pre-processing is done using the aroma.affymetrix R package. This procedure normalizes and summarizes the expression of the different probesets into single values.
...
<span style="color:red">**Note:**</span> For sequencing data, there are two simple requirements for the BAM files in order to get EventPointer working correctly:

1. The BAM files should include the **XS-flag**, the flag can be included in the files when running the alignment. Most of the available software has parameters to incude the flag.
