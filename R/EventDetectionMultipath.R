...
EventDetectionMultipath <- function(Input, cores, Path, paths = 2) {
    ...
    # Result <- vector(mode='list',length = length(genes)) for(jj in
    # seq_along(genes))
    Result <- foreach(jj = seq_along(genes)) %dopar% {
        ...
        if (nrow(as.data.frame(SG_Gene)) != 1) {
            ...
            A <- solve(diag(ncol(SG$Adjacency)) - SG$Adjacency) - diag(ncol(SG$Adjacency))
            A <- A[-ncol(A), -1]
            if (any(c(A[1, ], A[, ncol(A)]) == 0)) {
                warning("\nin Gene ", Gene, " the graph is not correct\n")
                ...
                if (any(round(randSol) != 1)) {
                  ...
                  # Events <- findTriplets(randSol) twop <- which(rowSums(Events$triplets !=0)==3)
                  
                  Events <- findTriplets2(SG$Incidence, paths = paths, randSol)
                  
                  # There should be al least one event to continue the algorithm
                  
                  # if (nrow(Events$triplets) > 0)
                  if (nrow(Events$multipaths) > 0) {
                    ...
                    twopaths <- which(rowSums(Events$multipaths != 0) == 4)
                    Events <- getEventMultiPaths(Events, SG, twopaths, paths)
                    ...
                    if (length(Events) > 0) {
                      ...
                      Events <- GetCountsMP(Events, SG_Gene_Counts)
                      ...
                      Info <- AnnotateEvents_RNASeq_MultiPath(Events, paths)
                      # browser()
                      
                      torome <- filterimagine(Info, paths)
                      ...
                      # Result[[jj]] <- list(Events = Events, Info = Info)
                      
                      # Eventss[[jj]]<-Events Infoss[[jj]]<-Info
