...
PSI_Statistic <- function(PSI, Design, Contrast, nboot) {
    ...
    table <- get_beta(combboots, incrPSI_original, ncontrastes)
    Pvalues <- table$pvalues
    
    # return(table) estimation of the lfdr. We need to remove the NaN
    if (nrow(Contrast) > 1) {
        LocalFDR <- apply(Pvalues, 2, function(X) {
            ...
            salida <- cobs(x = log(result$pvalues + eps), y = result$lfdr, constraint = "incr", 
                pointwise = matrix(c(-1, 0, 1, 1, min(log(result$pvalues + eps), 
                  na.rm = TRUE), 0), byrow = TRUE, ncol = 3), lambda = 1, print.mesg = FALSE)
            iix <- which(is.na(X))
            if (length(iix) > 0) {
                jjx <- which(!is.na(X))
                if (length(jjx) > 0) {
        ...
        salida <- cobs(x = log(LocalFDR$pvalues + eps), y = LocalFDR$lfdr, constraint = "incr", 
            pointwise = matrix(c(-1, 0, 1, 1, min(log(LocalFDR$pvalues + eps), na.rm = TRUE), 
                0), byrow = TRUE, ncol = 3), lambda = 1, print.mesg = FALSE)
        iix <- which(is.na(Pvalues))
        if (length(iix) > 0) {
            jjx <- which(!is.na(Pvalues))
            if (length(jjx) > 0) {
                LocalFDR$lfdr2 <- LocalFDR$lfdr
                LocalFDR$lfdr2[jjx] <- predict(salida, log(Pvalues[jjx] + eps))[, 
                  2]
            } else {
                LocalFDR$lfdr2 <- LocalFDR$lfdr
            }
    ...
    tablefinal <- list(deltaPSI = table$deltaPSI, Pvalues = Pvalues, LocalFDR = LocalFDR)
