...
CDFfromGTF<-function(input="Ensembl",inputFile=NULL,PSR,Junc,PathCDF,microarray="HTA-2_0")
{
  ...
  # CDFfromGTF is the corresponding function to create a CDF file based on
  # the GTF file provided that contains the "reference" trasncriptome to
  # be used to identify alternative splicing events.

  # Input Files:
  # input -> GTF File of the transcriptome
  # Dsource -> Information about the CDF (not required)
  # PSR -> Txt file with the information of the PSRs of the array
  # Junc -> Txt file with the information of junction probes in the array


  # Crate TxDB object that contains all the information contained in the gtf
  ...
  # Arrange the information in the txt file to have the information
  # in the way the algorithm needs it
  ...
  if(input=="Ensembl" | input=="UCSC")
  {
    ...
    GeN<-sapply(GeN,function(X){A<-X[1];return(A)})
  ...
  # Obtain the genenames("TCXXXX.hg") and geneIDs (1,2...) for all the genes in
  # the transcriptome used

  geneIds<-geneID(SplicingGraphFeatures)
  Genes<-as.list(geneName(SplicingGraphFeatures))

  # As certain genes share geneID we repeat each geneID as many
  # times it is used by a gene
  ...
  # For the probe variables, we only keep the
  # genes that appear in KeptGenes
  ...
  # Order all the variables in the same way, so
  # the genes in different variables have the same "index"
  Ord<-order(KeptGenes)
  ...
  # With rle we get how many times a certain value appears in the variable
  # this way we can get the indices that correspond to the probes for a particular
  # gene.
  ...
  for(jj in seq_len(nrow(GeneIndex)))
  {
    ...
    # while Junctions correspond to edges. It is important to take into account
    # that the Exons are already "divided" into subexons, non overlapping regions
    # between transcripts


    SG_Gene <- SplicingGraphFeatures[unlist(geneID(SplicingGraphFeatures))==GeneIndex[jj,2]]

    # Get the number of transcripts that appear for the gene that is
    # being analyzed

    Txx<-unique(unlist(as.data.frame(SG_Gene)[,"txName"]))

    # In order to have alternative splicing a gene must have
    # at least more than one exon and more than one transcript

    if (nrow(as.data.frame(SG_Gene))!=1 & length(Txx)!=1)
    {
      # Function to obtain the information of the SG, the information returned
      # Corresponds to the Edges, Adjacency matrix and Incidence Matrix of the
      # SG that is being analyzed

      SG<-SG_Info(SG_Gene)

      # Using Ax=b with A the incidence matrix and b=0, we solve
      # to obtain the null space and obtain the flux through each edge
      # if we relate the SG with an hydraulic installation

      # The new parameter (ncol) is used to generate 10 fluxes to ensure
      # that the eventdetection is exaclty the same every step.

      randSol <- getRandomFlow(SG$Incidence,ncol=10)

      # To find events, we need to have fluxes different from 1 in
      # different edges

      if(any(round(randSol)!=1))
      {
        # Identify the alternative splicing events using the fluxes
        # obtained previously. The number of fluxes correspond to the
        # number of edges so we can relate events with edges
        ...
        if(nrow(Events$triplets)>0)
        {
          ...
          # Transform events into triplets, related with edges to the corresponding
          # paths. Also the edges are divided into Path 1, Path 2 and Reference
          ...
          if(length(Events)>0)
          {
            ...
            # Classify the events into canonical categories using
            # the adjacency matrix

            Events<-ClassifyEvents(SG,Events)

            # Obtain the corresponding PSRs/JunctionProbes for
            # each of the paths of every event

            Events <- annotateEvents(Events,PSR_Gene,Junc_Gene,GeneIndex[jj,1])
  ...
  colnames(Result)<-c("Affy Gene Id","Gene Name","Event Number","Event Type","Genomic Position","Path 1","Path 2","Path Reference",
                      "Probes P1","Probes P2","Probes Ref")
  ...
  #   Result[,2]<-Nmb
  #
  # }else if(input=="Ensembl" | input=="UCSC")
  # {
  #   Result[,2]<-Result[,1]
  # }
