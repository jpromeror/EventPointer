...
EventPointer_RNASeq<-function(Events,Design,Contrast,Statistic="LogFC",PSI=FALSE)
{
  ...
  cat(paste("\t** Statistical Analysis: ",stt,sep=""),"\n")
  ...
  cat(paste(paste(rep(" ",length(unlist(strsplit(TimeS,"")))),sep="",collapse="")," ----------------------------------------------------------------",sep=""),"\n")
  ...
  if(Statistic=="LogFC" | Statistic=="Dif_LogFC" | Statistic=="DRS")
  {
    ...
    for(mm in 1:ncol(Contrast))
    {
      ...
      if(Statistic=="LogFC" | Statistic =="Dif_LogFC")
      {
        ...
        Final<-data.frame(Gen=rownames(T34_345),Pvalue=Values1$Pvalues,ZValue=Values1$Tstats,stringsAsFactors = FALSE)
      ...
      # Add extra information (Gene Name and Event Classification) and Sort data.frame by pvalue
