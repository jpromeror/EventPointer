...
EventDetection<-function(Input,cores,Path)
{
  ...
  IdName<-apply(GeneIDs,1,function(X){A<-unlist(X[2])[1];return(A)})
  GeneIDs[,2]<-IdName
    ...
    if(nrow(as.data.frame(SG_Gene))!=1)
    {
      ...
      if(any(round(randSol)!=1))
      {
        ...
        if(nrow(Events$triplets)>0)
        {
          ...
          # Transform triplets, related with edges to the corresponding
          # edges. Also the edges are divided into Path 1, Path 2 and Reference
          ...
          if(length(Events)>0)
          {
            ...
            # A simple piece of code to get the number of counts
            # for the three paths in every event within a gene

            Events <- GetCounts(Events,SG_Gene_Counts)

            Events<-Filter(Negate(is.null),Events)

            Events<-lapply(Events,function(x){x$GeneName<-GeneName;return(x)})

            Events<-lapply(Events,function(x){x$Gene<-Gene;return(x)})
  ...
  if(length(iix)>0)
  {
    Info<-matrix(unlist(strsplit(as.vector(TxtInfo[iix,1]),"_")),ncol=2,byrow=TRUE)[,1]
    TxtInfo[iix,2]<-Info

  }

  write.table(TxtInfo,file=paste(Path,"/EventsFound_RNASeq.txt",sep=""),sep="\t",row.names=FALSE,col.names=TRUE,quote=FALSE)
