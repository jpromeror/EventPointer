...
getbootstrapkallisto <- function(pathValues = NA, nb) {
    ...
    for (jj in seq_len(length(datafinal))) {
        ...
        for (iix in 0:(nb - 1)) {
            command <- paste0("abund_b", iix, " <- h5read(paste0(pathValues[jj],'/abundance.h5'),'bootstrap/bs", 
                iix, "')")
            # print(command)
        ...
        if (jj == 1) {
            # en la primera iteracion inicializar la matriz mymatrix y ponerle el rowname (el
            # resto tendran que tener el mismo orden)
            ...
            # comprobar que estan en el mismo orden para el resto de muestras
