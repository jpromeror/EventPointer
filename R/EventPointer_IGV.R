...
#'
#' @param Events Data.frame generated by EventPointer with the events to be included in the GTF file.
#' @param input Reference transcriprome. Available options are : Ensembl, UCSC and GTF.
...
#'
#' @return The function displays a progress bar to show the user the progress of the function. Once the progress bar reaches 100%, two .gtf
#' files are written to the specified directory in PathGTF. The created files are: 1) paths.gtf : GTF file representing the alternative splicing
#' events and 2) probes.gtf : GTF file representing the probes that measure each event and each path.
#'
#' @examples
#'
...
EventPointer_IGV<-function(Events,input,inputFile=NULL,PSR,Junc,PathGTF,EventsFile)
{

  # EventPointer_IGV is the function to obtain the IGV visualization
  # for the detected Events using EventPointer. Compared to previous
  # versions, this time we will not need to load any .Rdata files
  # in order to create the gtf files, as a result certain functions
  ...
  iix<-match(matrix(unlist(strsplit(rownames(Events),"_")),ncol=2,byrow=TRUE)[,1],GeneIndex[,1])
  GeneIndex<-GeneIndex[iix,,drop=FALSE]
  ...
  # Create file to store gtf for probes (events)
  ...
  for(jj in seq_len(nrow(GeneIndex)))
  {
    ...
    IDs<-c(rep("Path1",length(xProbe.P1)),rep("Path2",length(xProbe.P2)),rep("Ref",length(xProbe.R)))
    ...
    class(EventPaths[,2])<-"character"
    class(EventPaths[,3])<-"character"
