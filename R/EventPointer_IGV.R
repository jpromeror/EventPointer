...
EventPointer_IGV <- function(Events, input, inputFile = NULL, PSR, Junc, PathGTF, 
    EventsFile,microarray=NULL) {
    
    # EventPointer_IGV is the function to obtain the IGV visualization for the
    # detected Events using EventPointer. Compared to previous versions, this time we
    # will not need to load any .Rdata files in order to create the gtf files, as a
    # result certain functions might need to be called again.
  ...
  }else if(microarray=="MTA")
  {
    ...
    # iix <- match(matrix(unlist(strsplit(rownames(Events), "_")), ncol = 2, byrow = TRUE)[, 
    #     1], GeneIndex[, 1])
    
    EvId<-strsplit(rownames(Events),"_")
    EvNum<-sapply(EvId,tail,1)
    Idmat<-cbind(as.vector(Events[,1]),EvNum)
    ...
    cat(file = FILE.paths, paste("#track name=", shQuote("probes", type = "cmd"), 
        " gffTags=", shQuote("on", type = "cmd"), sep = ""), "\n")
    ...
    for (jj in seq_len(nrow(GeneIndex))) {
        ...
        JnProbes[, "Start"] <- round((as.numeric(JnProbes[, "Start"]) + as.numeric(JnProbes[, 
            "Stop"]))/2)
        ...
        IDs <- c(rep("Path1", length(xProbe.P1)), rep("Path2", length(xProbe.P2)), 
            rep("Ref", length(xProbe.R)))
        Wds <- rep(25, length(IDs))
        EventProbes <- cbind(EventProbes[c(ii.P1, ii.P2, ii.PR), c(1, 5, 6)], Wds, 
            EventProbes[c(ii.P1, ii.P2, ii.PR), 8], IDs)
        colnames(EventProbes) <- c("names", "chromosome", "start", "width", "strand", 
            "Path")
